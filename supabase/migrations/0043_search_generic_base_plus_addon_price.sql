-- Quando o cardápio modela uma pizza como produto genérico ("Pizza P", "Pizza G")
-- e o sabor como adicional/opção ("Calabresa + R$ 10"), a busca deve poder exibir
-- "Pizza Calabresa" com preço final estimado = preço base + adicional.
-- A regra é intencionalmente limitada a bases genéricas para não transformar
-- "adicionar calabresa na Pizza 4 Queijos" em um produto falso.
...
CREATE OR REPLACE FUNCTION public.search_menu_items(
 ...
 option_matches AS (
   SELECT
       mi.id AS item_id,
       CASE
         WHEN flags.is_addon AND flags.is_generic_pizza_base THEN
           trim(CONCAT('Pizza ', regexp_replace(mio.name, '^(ADICIONAL|ADD|EXTRA)[[:space:]]+', '', 'i')))
         ELSE mio.name
       END AS item_name,
       CASE
         WHEN flags.is_addon AND flags.is_generic_pizza_base THEN
       ...
       CASE
         WHEN flags.is_addon AND flags.is_generic_pizza_base THEN 'base_plus_addon_variant'
         ELSE 'option_variant'
       END::text AS item_commercial_type,
       ...
       r.neighborhood AS restaurant_neighborhood,
       r.opening_hours AS restaurant_opening_hours,
       CASE WHEN flags.is_addon AND flags.is_generic_pizza_base THEN 1 ELSE 2 END AS match_rank,
       mi.order_index AS order_index
   ...
   CROSS JOIN LATERAL (
     SELECT
       lower(extensions.unaccent(CONCAT_WS(' ', mio.name, mio.group_name, mi.name, mi.display_name, mc.name))) AS search_blob,
       lower(extensions.unaccent(COALESCE(mio.group_name, ''))) AS group_blob,
       lower(extensions.unaccent(COALESCE(mio.name, ''))) AS option_blob,
       lower(extensions.unaccent(COALESCE(mi.display_name, mi.search_display_name, mi.name, ''))) AS base_blob
   ) sb
   CROSS JOIN LATERAL (
     SELECT
       (sb.group_blob LIKE '%adicional%' OR sb.group_blob LIKE '%acrescimo%' OR sb.group_blob LIKE '%extra%' OR sb.option_blob LIKE 'add %' OR sb.option_blob LIKE 'adicional %') AS is_addon,
       (
         sb.base_blob ~ '^pizzas?[[:space:]]*(p|m|g|media|m[eé]dia|grande|pequena|broto|[0-9]|$)'
         OR sb.base_blob LIKE '%monte%sua%pizza%'
       ) AS is_generic_pizza_base
   ) flags
   WHERE
       ...
       AND (
         flags.is_addon = FALSE
         OR flags.is_generic_pizza_base = TRUE
       )
       AND sb.group_blob NOT LIKE '%embalagem%'
       AND sb.group_blob NOT LIKE '%azeitona%'
       AND sb.group_blob NOT LIKE '%leite%'
       AND sb.group_blob NOT LIKE '%molho%'
       AND sb.group_blob NOT LIKE '%observa%'
 )
 SELECT
   item_id,
   item_name,
