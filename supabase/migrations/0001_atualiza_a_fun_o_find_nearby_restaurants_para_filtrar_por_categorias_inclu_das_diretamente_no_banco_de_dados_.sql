CREATE OR REPLACE FUNCTION public.find_nearby_restaurants(
 ...
 RETURNS TABLE(id uuid, user_id uuid, name text, description text, image_url text, cover_image_url text, plan restaurant_plan, created_at timestamp with time zone, latitude numeric, longitude numeric, category text, city text, state text, distance_km numeric)
 LANGUAGE sql
AS $function$
SELECT
    r.id,
