-- Metadados semânticos de opções gerados por heurística + IA no salvamento do cardápio.
-- A busca usa estes campos para saber quando uma opção é sabor pesquisável,
-- adicional comum, diferença de preço, preço final absoluto ou item não pesquisável.
...
ALTER TABLE public.menu_item_options
  ...
  ADD COLUMN IF NOT EXISTS is_searchable_variant boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS ai_confidence numeric,
  ADD COLUMN IF NOT EXISTS ai_reason text;

CREATE INDEX IF NOT EXISTS idx_menu_item_options_semantic_search
  ON public.menu_item_options(semantic_type, is_searchable_variant)
  WHERE is_searchable_variant = true;
