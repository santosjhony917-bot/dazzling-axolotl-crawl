...
async function run() {
  ...
  try {
    ...
    console.log("Visualizador aberto. Iniciando loop de extração clicando no botão Seguinte...");
    
    const results = [];
    
    for (let i = 0; i < 15; i++) {
      const data = await page.evaluate(() => {
        // 1. Encontra a imagem principal
        let imgUrl = null;
        const viewerImages = Array.from(document.querySelectorAll('img[src*="googleusercontent.com"]'));
        // A imagem principal no visualizador é normalmente grande
        ...
        // Tentamos spans específicos primeiro
        const lg5SpSpans = Array.from(document.querySelectorAll('span.lg5Sp'));
        if (lg5SpSpans.length > 0) {
          // No visualizador, o texto da data costuma ser o primeiro span com a classe lg5Sp
          dateText = lg5SpSpans[0].textContent.trim();
        }
        
        if (!dateText) {
          // Busca geral em spans/divs com base em palavras de tempo
        ...
        // 3. Tenta clicar no botão Seguinte
      ...
      console.log(`Foto ${i+1}: URL = ${data.imgUrl ? data.imgUrl.substring(0, 80) : 'null'}, Data = ${data.dateText}, ClicouNext = ${data.clickedNext}`);
      results.push(data);
