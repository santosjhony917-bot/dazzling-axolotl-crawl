import React, { memo } from 'react';
import { Crown, Heart, Home, Search, Sparkles, User } from 'lucide-react';
import { Link, useLocation } from 'react-router-dom';
import { CurvedBottomNav, curvedCenterActionClassName } from '@/components/navigation/CurvedBottomNav';
import { createPageUrl } from '@/utils/url';

const RestaurantBottomNav = memo(({ isFree }: { isFree: boolean }) => {
  const location = useLocation();
  const ownerProfilePath = createPageUrl('restaurant-area/profile-menu');
  const isActive = (path: string) => {
    const current = location.pathname.replace(/\/$/, '');
    const target = path.replace(/\/$/, '');
    return current === target || (target === ownerProfilePath && current.startsWith(target));
  };

  const centerAction = isFree ? (
    <Link
      to={createPageUrl('restaurant-area/upgrade')}
      aria-label="Ver planos premium"
      className={curvedCenterActionClassName}
    >
      <Crown className="h-7 w-7 stroke-[3]" aria-hidden="true" />
      <span className="absolute -bottom-[19px] left-1/2 -translate-x-1/2 whitespace-nowrap text-[8px] font-extrabold text-highlight">
        Premium
      </span>
    </Link>
  ) : (
    <Link
      id="tour-ai-button"
      to="/home?assistant=1"
      aria-label="Fazer uma pergunta à IA dos cardápios"
      className={curvedCenterActionClassName}
    >
      <Sparkles className="h-7 w-7 stroke-[3]" aria-hidden="true" />
      <span className="absolute -bottom-[19px] left-1/2 -translate-x-1/2 whitespace-nowrap text-[8px] font-extrabold text-highlight">
        Perguntar
      </span>
    </Link>
  );

  return (
    <CurvedBottomNav
      ariaLabel="Navegação principal do restaurante"
      indicatorLayoutId="restaurant-nav-indicator"
      items={[
        { icon: Home, path: '/home', label: 'Início', isSelected: isActive('/home') },
        { icon: Search, path: '/search', label: 'Explorar', isSelected: isActive('/search') },
        { icon: Heart, path: '/favorites', label: 'Favoritos', isSelected: isActive('/favorites') },
        { icon: User, path: ownerProfilePath, label: 'Perfil do restaurante', shortLabel: 'Perfil', isSelected: isActive(ownerProfilePath) },
      ]}
      centerAction={centerAction}
    />
  );
});

RestaurantBottomNav.displayName = 'RestaurantBottomNav';

export default RestaurantBottomNav;
