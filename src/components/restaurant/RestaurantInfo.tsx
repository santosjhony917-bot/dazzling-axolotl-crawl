...
const RestaurantInfo: React.FC<RestaurantInfoProps> = memo(({ restaurant }) => {
  ...
  return (
    <div className="mt-8">
      <h2 className="text-lg font-bold text-primary">Informações Adicionais</h2>
      <div className="mt-4 space-y-4">
        
        {/* Payment Methods */}
        <div className="flex items-start gap-3">
          <CreditCard className="w-5 h-5 text-highlight pt-1 shrink-0" />
          <div>
            ...
            <div className="flex flex-wrap items-center gap-3 mt-2">
              {mockPaymentMethods.map((method, index) => {
                ...
                return (
                  <div key={index} className="flex items-center gap-2 rounded-full border border-gray-200 bg-white px-3 py-1 shadow-none">
