...
const SubscriptionCard: React.FC<SubscriptionCardProps> = ({ restaurant }) => {
  ...
  const getPlanDetails = () => {
    switch (plan) {
      case 'premium':
        return {
          title: 'Premium (Assinante)',
          description: 'Acesso total a todos os recursos com assinatura ativa.',
          color: 'text-yellow-600',
          icon: <CheckCircle className="w-5 h-5 text-green-500" />,
        };
      case 'premium_gift':
        return {
          title: 'Premium (Cortesia)',
          description: 'Acesso total cortesia aos recursos do aplicativo.',
