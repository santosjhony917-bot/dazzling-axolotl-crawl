...
const RestaurantProfileHeader: React.FC<RestaurantProfileHeaderProps> = ({ restaurant }) => {
  ...
  return (
    <div className={cn("relative w-full", isCompact ? "h-24" : (isPremium ? "h-48" : "h-0"))}>
      ...
      <div className="absolute top-0 left-0 right-0 p-4 flex justify-between items-start">
        <Button variant="ghost" size="icon" onClick={() => navigate(-1)} className="bg-white/50 backdrop-blur-sm rounded-full"> {/* Usando navigate(-1) para voltar */}
