...
const PremiumProfileLayout: React.FC<PremiumProfileLayoutProps> = ({ restaurant, toggleFavorite, isFavoriteMutating, isCompact = false }) => {
  ...
  const [activeTab, setActiveTab] = useState<'menu' | 'gallery' | 'info'>(
    restaurant.gallery_images && restaurant.gallery_images.length > 0 ? 'gallery' : 'menu'
  );
  ...
  // Sequência ordenada das abas disponíveis
  const tabsOrder = useMemo(() => {
    const order: Array<'gallery' | 'menu' | 'info'> = [];
    if (hasGallery) order.push('gallery');
    if (hasMenu) order.push('menu');
    if (hasInfo) order.push('info');
    return order;
