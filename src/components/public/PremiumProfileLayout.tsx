...
import { MapPin, Clock, Phone, Mail, Globe, ExternalLink, MessageSquare, UtensilsCrossed, Star } from 'lucide-react';
...
const PremiumProfileLayout: React.FC<PremiumProfileLayoutProps> = ({ restaurant, menuCategories, galleryImages }) => {
  ...
  return (
    <div className="container mx-auto p-4">
      {/* Cover Image */}
      <div className="relative h-64 md:h-80 bg-gray-200 rounded-lg overflow-hidden shadow-md mb-6">
        <img
        ...
        <div className="absolute bottom-6 left-6 text-white">
          ...
          {restaurant.category && <p className="text-xl">{restaurant.category}</p>}
          <div className="flex items-center mt-2">
            <Star className="h-5 w-5 fill-yellow-400 text-yellow-400 mr-1" />
            <span className="text-lg font-semibold">4.8 (250 avaliações)</span> {/* Placeholder for rating */}
          </div>
        </div>
      </div>
