...
import { cn } from '@/lib/utils'; // Adicionado: Importação do utilitário cn
...
// Nova interface para as props do componente RestaurantProfilePublic
...
const RestaurantProfilePublic: React.FC<RestaurantProfilePublicProps> = ({ initialRestaurantId, simulatedPlan, isCompact }) => {
  ...
  const restaurantId = initialRestaurantId || id; // Usa initialRestaurantId se fornecido, senão usa o da URL
  ...
  const toggleFavoriteMutation = useMutation<void, Error, boolean>({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['isFavorite', restaurantId, userId] });
      queryClient.invalidateQueries({ queryKey: ['followerCount', restaurantId] });
  ...
  // Usa simulatedPlan se fornecido, caso contrário, usa o plano real do restaurante
    ...
    <div className="min-h-screen bg-gray-50">
      {isPremium ? (
        <PremiumProfileLayout
          restaurant={restaurant} // Removido o cast, agora o tipo é compatível
          toggleFavorite={handleToggleFavorite}
          isFavoriteMutating={toggleFavoriteMutation.isPending}
          isCompact={isCompact || false} // Passa isCompact
